...
// }
  ...
  deleteImage(_id, e){
    axios.delete(`http://localhost:8000/image/element/${_id}`)
      .then(res => {
        console.log(res);
      ...
      <table className="image-table">
           ...
           <tbody>
              {this.state.posts.map((post) => (
                <tr>
                  <td>{post._id}</td>
                  <td><img alt="Source-products" className="td-image" src={API_URL+"/"+post.imageData } /></td>
                  <td>
                    <button className="btn" onClick={(e) => this.deleteImage(post._id, e)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
